...
import { Home, LayoutGrid, ShoppingBag, Menu, Search, Bell, LogOut, User, AlertTriangle, CheckCircle2, Info } from "lucide-react";

const NAV = [
  { to: "/dashboard", label: "Dashboard", icon: Home },
  { to: "/yayasan", label: "Registered Foundations", icon: LayoutGrid },
  { to: "/kampanye", label: "Campaign Approvals & Overview", icon: ShoppingBag },
];

export default function DinsosLayout({ children }) {
    ...
    <div className="flex flex-col h-screen bg-slate-50 font-sans text-slate-900 selection:bg-teal-500/30">
      ...
      <header className="h-[64px] bg-white border-b border-slate-200 flex items-center justify-between px-6 shrink-0 z-20 sticky top-0">
        <div className="flex items-center">
          ...
          <div className="font-extrabold text-xl tracking-tight text-slate-900 sm:ml-4 md:ml-0">
            TrustFund.
          </div>
        </div>
              ...
              {isProfileOpen && (
                <div className="absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-lg border border-slate-200 py-1.5 z-50 animate-fade-in-up [animation-duration:200ms]">
                  <div className="px-4 py-2.5 border-b border-slate-100/80 mb-1">
                    ...
                    <p className="text-[11px] font-medium text-slate-500 mt-0.5 truncate">{user?.email || "demo@trustfund.com"}</p>
                  </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className="w-[260px] bg-white flex flex-col py-6 shrink-0 border-r border-slate-200 hidden md:flex">
          <div className="px-5 mb-3 text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Main Menu</div>
